...
bool getUserCredentials(sqlite3* db, const string& username, string& passwordHash, string& salt, int& userId) {
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        passwordHash = string(reinterpret_cast<const char*>(sqlite3_column_text(stmt, 1)));
        salt = string(reinterpret_cast<const char*>(sqlite3_column_blob(stmt, 2)), sqlite3_column_bytes(stmt, 2));
        sqlite3_finalize(stmt);
        return true;
    } else {
...
bool storeWebsitePassword(sqlite3* db, const string& website, const string& password, const string& masterKey) {
    ...
    string ivStr(reinterpret_cast<char*>(iv), AES_BLOCK_SIZE);
